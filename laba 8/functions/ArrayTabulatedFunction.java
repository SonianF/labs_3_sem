...
public class ArrayTabulatedFunction implements TabulatedFunction, java.io.Serializable, Cloneable {
    ...
    public String toString() {
        ...
        str.append(values[countOfPoint-1].toString());
        str.append("}");
        return str.toString();
    }

    public boolean equals(Object o) {
        if (!(o instanceof TabulatedFunction)) return false; //если не реализуется TabulatedFunction;
        if (o.getClass()==getClass()) { //если о - ArrayTabuletedFunction
            if(((ArrayTabulatedFunction)o).countOfPoint!=countOfPoint) return false; //сравниваем количество точек
            for (int i=0; i<countOfPoint; ++i) { //сравниваем сами точки
                if(!values[i].equals(((ArrayTabulatedFunction)o).values[i])) {
                    return false;
                }
            }
            return true;
        ...
        for (int i=0; i<countOfPoint; ++i) { //сравниваем сами точки
            if (!values[i].equals(((TabulatedFunction)o).getPoint(i)));
            return false;
        }
        return true; //если мы здесь, то все точки совпали
    }
...
public Object clone() {
       ...
       try {
           result = super.clone();
           ((ArrayTabulatedFunction)result).values = values.clone();
           for (int i=0; i<countOfPoint; ++i) { //клонируем каждую точку
               ((ArrayTabulatedFunction) result).values[i]= (FunctionPoint) values[i].clone();
           }
       }
